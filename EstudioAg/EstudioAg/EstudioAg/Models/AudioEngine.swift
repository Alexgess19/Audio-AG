...
/**
 * EstudioAg - AudioEngine.swift (Arquitectura BROADCAST v22.0 - HEADLESS RT)
 * 
...
final class RenderChannel: @unchecked Sendable {
    ...
    /// [FIX-C] Marca que la adaptación de latencia al chip físico ya se ejecutó.
    /// Evita que updateThresholds/resetToTarget descarte frames encolados en cada callback.
    var isAdaptedToChip: Bool = false
...
final class AudioEngineCore: @unchecked Sendable {
    ...
    init() {
        ...
        // ANTES de iniciar. Esto evita que AVAudioEngine use drivers virtuales (EstudioAg, Estudio Ag Input)
        ...
        if let builtInID = AudioEngineCore.findBuiltInOutputDeviceID(),
           let au = engine.outputNode.audioUnit {
            ...
            // [FIX-10874] Aumentar mMaxFramesPerSlice a 8192 en el AUHAL interno de AVAudioEngine.
            // Si el dispositivo físico cambia su tamaño de búfer a 1024 frames (o más) después
            // de inicializado el engine, este colapsaría con el error kAudioUnitErr_TooManyFramesToProcess
            // porque su capacidad máxima por defecto podría haber quedado en 128 o 512.
            var maxFrames: UInt32 = 8192
            AudioUnitSetProperty(au, kAudioUnitProperty_MaximumFramesPerSlice, kAudioUnitScope_Global, 0, &maxFrames, UInt32(MemoryLayout<UInt32>.size))
        }
...
class RadioAudioEngine: ObservableObject {
    ...
    @Published public var isEngineStarted = true; @Published public var isOnAir = false {
        didSet {
            ...
            if isOnAir {
                onAirStartDate = Date()
                // [FIX-D] Prevenir suspensión de la pantalla y el sistema durante transmisión en vivo.
                // ScreenCaptureKit detiene la captura (incluso de solo audio) si la pantalla entra en reposo.
                // Usamos PreventUserIdleDisplaySleep para mantener activo el servidor de ventanas.
                IOPMAssertionCreateWithName(
                    kIOPMAssertionTypePreventUserIdleDisplaySleep as CFString,
                    IOPMAssertionLevel(kIOPMAssertionLevelOn),
                    "EstudioAg: Transmisión en vivo activa" as CFString,
                    &powerAssertionID
                )
                broadcastActivity = ProcessInfo.processInfo.beginActivity(
                    options: [.latencyCritical, .idleSystemSleepDisabled],
                    reason: "EstudioAg: Radio broadcast en vivo"
                )
                print("🔒 [FIX-D] Power assertion activa: el sistema no se suspenderá durante la transmisión.")
            } else {
                onAirStartDate = nil
                // [FIX-D] Liberar las assertions al salir del aire
                if powerAssertionID != 0 {
                    IOPMAssertionRelease(powerAssertionID)
                    powerAssertionID = 0
                }
                if let activity = broadcastActivity {
                    ProcessInfo.processInfo.endActivity(activity)
                    broadcastActivity = nil
                }
                print("🔓 [FIX-D] Power assertion liberada.")
            }
        }
    }
    ...
    // [FIX-D] Power assertions: previenen App Nap y suspensión del sistema durante transmisión.
    private var powerAssertionID: IOPMAssertionID = 0
    private var broadcastActivity: NSObjectProtocol?
    ...
    public init() {
        ...
        // [FIX-E] passRetained: incrementa el retain count de RadioAudioEngine, garantizando que
        // el puntero sea válido cuando CoreAudio dispare el listener (ej: al encenderse la pantalla).
        // passUnretained anterior podía dejar un dangling pointer si el sistema hacía un ciclo de
        // memoria agresivo durante la suspensión → crash o dispositivos no reconocidos al reactivar.
        ...
        HardwareCaptureManager.shared.onAudioBuffer = { [weak self] (id, buffer) in
            ...
            if let ch = self.core.registry.getChannel(id) {
                // [FIX-C] Adaptación de latencia: solo se ejecuta UNA vez por canal/dispositivo.
                // La condición anterior (targetLatency != 128) se evaluaba en cada callback;
                // updateThresholds() llama internamente a resetToTarget() que reposiciona el tail
                // del ring buffer, descartando frames encolados y causando micro-cortes audibles
                // al reconectar dispositivos. El flag isAdaptedToChip resuelve esto.
                if ch.type == .mic && !ch.isAdaptedToChip {
                    let newTarget = max(frameLength, 128)
                    let newMax = newTarget * 8
                    ch.queue.updateThresholds(target: newTarget, max: newMax)
                    ch.isAdaptedToChip = true
                    print("⚡️ [FIX-C] Canal Mic '\(ch.name)' adaptado al chip: \(newTarget) frames (~\(String(format: "%.2f", Double(newTarget)/48.0))ms)")
                }
    ...
    public func setSource(_ name: String, for id: UUID, type: ChannelType, voiceIsolation: Bool = false) -> AudioSourceID {
        ...
        ch?.queue.clear()
        ch?.isAdaptedToChip = false  // [FIX-C] Permite re-adaptación al nuevo dispositivo
...
struct AudioDeviceManager {
    ...
    mutating func scanDevices() {
        ...
        for id in ids {
            ...
            if isInput {
                ...
                // Los drivers virtuales (HALC_ProxyIOContext: CASTER, Teams, Estudio Ag Input, etc.)
    ...
    /// como CASTER, Microsoft Teams, Estudio Ag Input, Estudio Ag Output, CADefaultDeviceAggregate, etc.
