...
class CaptureSession {
    ...
    func start() {
        ...
        // [FIX-A] Forzar el sample rate del dispositivo analógico a 48kHz antes de iniciar el AUHAL.
        // Si el chip trabaja a 44.1kHz y el engine a 48kHz, CoreAudio no puede convertir internamente
        // → audio distorsionado (efecto robótico) o silencio. Forzamos la tasa del dispositivo
        // para que el AUHAL opere en el mismo dominio de clock que el engine.
        if nativeSampleRate != 48000.0 {
            var targetRate: Float64 = 48000.0
            ...
            AudioObjectIsPropertySettable(device.id, &rateAddr, &settable)
            if settable.boolValue {
                AudioObjectSetPropertyData(device.id, &rateAddr, 0, nil,
                    UInt32(MemoryLayout<Float64>.size), &targetRate)
                // Dar tiempo al driver para estabilizar el nuevo clock antes de iniciar el AUHAL
                Thread.sleep(forTimeInterval: 0.08)
                print("🔧 [FIX-A] Sample rate de '\(device.name)' forzado a 48kHz (era \(nativeSampleRate)Hz)")
            } else {
                print("⚠️ [FIX-A] '\(device.name)' no permite cambiar sample rate (\(nativeSampleRate)Hz). Puede haber distorsión de pitch.")
            }
        }
