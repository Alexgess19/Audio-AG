...
La arquitectura de **Estudio Ag Suite** está diseñada en torno a la optimización extrema del procesamiento de señales en tiempo real (DSP) y el aislamiento de subprocesos para garantizar un flujo continuo y determinista de audio de calidad broadcast en macOS.
...
En la radiodifusión en vivo, la latencia debe situarse idealmente por debajo de los 20ms para evitar efectos de desfase acústico perceptibles por el locutor. **Estudio Ag Suite** implementa un motor híbrido optimizado:
...
1. **`Estudio Ag Input` (Driver de Transmisión)**:
...
2. **`Estudio Ag Output` (Driver de Monitoreo PFL)**:
...
* **Detección Dinámica**: El script `Instalador Audio Ag/create_full_installer.sh` busca automáticamente en tu Llavero local un certificado válido del Apple Developer Program (`Apple Development`).
...
### 5.2 Proceso de Instalación Hermético (`Instalador Audio Ag/create_full_installer.sh`)
...
2. **Fase de Despliegue**: Copia la aplicación en `/Applications/Estudio Ag/` y coloca los drivers HAL en el directorio del sistema.
...
* **AL AIRE (Rojo Brillante)**: La transmisión general está habilitada y todo el audio mezclado se envía al driver virtual `Estudio Ag Input`.
...
1. Cierra **Estudio Ag** por completo (`Cmd + Q`).
2. Abre la **Terminal** y limpia el registro de ScreenCaptureKit ejecutando:
   ```bash
   tccutil reset ScreenCapture
   ```
3. Ejecuta el nuevo instalador actualizado: [**`EstudioAg_Installer.pkg`**](file:///Users/alex/Proyectos%20anexos/estudio-ag/EstudioAg_Installer.pkg)
4. Abre la consola, ve a *Configuración del Sistema > Privacidad y seguridad > Grabación de pantalla*, apaga y **vuelve a encender el interruptor de "Estudio Ag"** para asegurar que registre tu nueva firma oficial de desarrollador.
...
* **Solución**: Abre la aplicación integrada de macOS **Configuración de Audio MIDI** y asegúrate de que todos tus dispositivos físicos y virtuales (`Estudio Ag Input`, `Estudio Ag Output`, Bocinas y Micrófonos) estén configurados de forma estándar a **48,000 Hz (48kHz)**.
...
* 🎛️ **[AudioEngine.swift](file:///Users/alex/Proyectos%20anexos/estudio-ag/EstudioAg/EstudioAg/EstudioAg/Models/AudioEngine.swift)**: El núcleo DSP, la configuración de buses en tiempo real y el inicio del motor de audio.
* 🎙️ **[AppCaptureManager.swift](file:///Users/alex/Proyectos%20anexos/estudio-ag/EstudioAg/EstudioAg/EstudioAg/Models/AppCaptureManager.swift)**: La lógica de captura asíncrona de aplicaciones vía ScreenCaptureKit.
* 🎚️ **[Components.swift](file:///Users/alex/Proyectos%20anexos/estudio-ag/EstudioAg/EstudioAg/EstudioAg/Views/Components.swift)**: Elementos de UI reutilizables (RangeSliders, perillas interactivas, scroll nativo).
* ⚙️ **[Instalador Audio Ag/create_full_installer.sh](file:///Users/alex/Proyectos%20anexos/estudio-ag/Instalador Audio Ag/create_full_installer.sh)**: Script automatizado de compilación, firma y creación del instalador del sistema.

---
*Manual técnico desarrollado para la Consola Profesional de Transmisión Estudio Ag Suite. Todos los derechos reservados. 📻*
